...
namespace ZeroconfService
{
    ...
    public sealed class NetService : DNSService
    {
            ...
            IntPtr context)
        {
            ...
            c.mPort = port;//(int)System.Net.IPAddress.NetworkToHostOrder(port);
            ...
            IAsyncResult ar = System.Net.Dns.BeginGetHostEntry(c.HostName, cb, c);
        ...
        {
            get { 
                //Why does Windows 7 not resolve host names that end with "local."? It resolves "HostName." just fine...
                if(mHostName.EndsWith("local."))
                    return mHostName.Replace("local.", "");
                return mHostName; 
            }
        }
